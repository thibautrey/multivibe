...
const env={MANAGED_INJECTOR_PROVIDER_MANIFEST_FILE:"/config/providers.json",MANAGED_INJECTOR_COORDINATION_URL:"https://coordination.internal",
...
test("injector has separate mandatory credential, coordination and workload configuration",async()=>{
 ...
 assert.throws(()=>loadManagedInjectorRuntimeConfig({...env,MANAGED_INJECTOR_COORDINATION_URL:"http://coordination"}));
 await assert.rejects(createManagedInjectorRuntime(config),/ENOENT/);
});
