...
export function rankOpenModels(catalog: OpenModelCatalog, need: CatalogNeed, sort: CatalogSort, estimates: RuntimeEstimate[] = [], now = Date.now(), evidence?: RecommendationEvidence) {
  let rows = groupModels(catalog.models).filter(g => g.model.needs.includes(need) || (g.familyStatus === 'resolved' && g.variants.some(v=>v.needs.includes(need)))).map(g => {
    const variants = g.variants.map(model => {
      ...
      const runtime = runtimeMemory(estimate, evidence?.memory);
      const predictions = evidence?.artifactMemory?.get(model.id) ?? (evidence?.discoveryMemory?.has(model.id)?[evidence.discoveryMemory.get(model.id)!]:[]);
      const availability = evidence?.memory;
      const shared = availability?.accelerator === 'metal' || availability?.accelerator === 'cpu';
      const limits = [availability?.budgetMiB, shared ? availability?.freeHostMiB : undefined].filter((n): n is number=>typeof n === 'number' && Number.isFinite(n) && n >= 0);
      const limit = limits.length ? Math.min(...limits) : undefined;
      const prediction = predictions.find(p=>limit!==undefined&&p.requiredMiB<limit) ?? predictions[0];
      ...
      return { model, memory, estimateStatus:evidence?.memoryReports?.[model.id]?.reason ?? 'queued', estimateVariant: memory.artifact, compatibility: memory.state, reason: estimate?.reason ?? 'Host has no memory estimate for this variant. Discovery alone cannot confirm that it fits or can be installed.' };
    });
