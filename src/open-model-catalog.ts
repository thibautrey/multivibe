...
export function parseOpenModels(value: unknown): OpenModel[] {
  ...
  return value.flatMap(item => {
    ...
    const tags = strings(item.tags); const card = item.cardData ?? {};
...
export function createOpenModelCatalog(fetcher: typeof fetch = fetch, now = Date.now, cachePath?: string) {
  let cache: OpenModelCatalog | undefined; let pending: Promise<OpenModelCatalog> | undefined; let hydrated = false;
  ...
  async function load(): Promise<OpenModelCatalog> {
    if (!hydrated) { hydrated = true; if (cachePath) try { const saved = JSON.parse(await fs.readFile(cachePath,'utf8')); if (saved.version === '2' && Array.isArray(saved.models) && Number.isFinite(Date.parse(saved.checkedAt)) && saved.models.every((m: OpenModel) => typeof m.id === 'string' && idPattern.test(m.id) && m.url === `${source}/${m.id}` && Array.isArray(m.needs) && Array.isArray(m.files))) cache = saved; } catch { /* First run or invalid cache. */ } }
