...
export function quantizationLabel(name: string): string | null {
  return name.match(/(?:^|[.\/_-])((?:IQ|Q)[2-8](?:_[A-Z0-9]+)*|BF16|FP16|F16|FP8|INT[248]|[248]-?BIT)(?=[.\/_-]|$)/i)?.[1]?.toUpperCase() ?? null;
}
export function modelArtifacts(model: OpenModel): ModelArtifact[] {
  const gguf = model.files.filter(file => /\.gguf$/i.test(file.name) && !/mmproj/i.test(file.name));
  ...
  return [{name:model.id,format:safetensors.length?'Safetensors':weights.length?'PyTorch':model.formats.join(', ') || 'Unknown',quantization:model.quantization ?? quantizationLabel(model.id) ?? 'Unknown',bytes:weights.length && complete && weights.every(f=>f.bytes!==null)?weights.reduce((n,f)=>n+f.bytes!,0):null,files:weights.map(f=>f.name)}];
}
