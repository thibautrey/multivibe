...
test('pagination, deduplication and new model discovery without code changes',async()=>{
 ...
 assert.equal((await load()).models.length,1);assert.equal(calls,44);
...
test('atomic cache survives restart and stale failure; no weights fetched',async()=>{
 ...
 const load=createOpenModelCatalog(fetcher,()=>now,file);await Promise.all([load(),load()]);assert.equal(calls,22);
