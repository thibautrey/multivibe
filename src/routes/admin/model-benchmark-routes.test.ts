...
async function fixture(t: any) {
  ...
  const app = express(); app.use("/admin/benchmarks", modelBenchmarkRoutes(createModelBenchmarkClient({ fetcher, artificialAnalysisApiKey: "test" })));
...
test("dashboard benchmark endpoints expose source, model, leaderboard and optional catalog contracts", async t => {
  ...
  for (const path of ["/sources", "/models?model=owner%2Fmodel", "/leaderboards?dataset=org%2Fbench&limit=1", "/artificial-analysis/models?page=1"]) {
