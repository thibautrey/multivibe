...
export class TeamMachineSharing {
  ...
  async execute(secret:string,transport:'private_network'|'cloud_relay',requestPath:string,body:any,signal:AbortSignal) {
    ...
    try {
      ...
      let usage: {input:number;output:number;cached:number;reasoning:number}|undefined;
      let pending='',firstByteAt:number|undefined;const decoder=new TextDecoder();const streaming=response.headers.get('content-type')?.includes('text/event-stream');
      const inspect=(line:string)=>{try{const value=JSON.parse(line.replace(/^data:\s*/,''));const u=value.usage??value.response?.usage;if(!u)return;const number=(v:unknown)=>Number.isSafeInteger(v)&&Number(v)>=0?Number(v):0;usage={input:number(u.prompt_tokens??u.input_tokens),output:number(u.completion_tokens??u.output_tokens),cached:number(u.prompt_tokens_details?.cached_tokens??u.input_tokens_details?.cached_tokens),reasoning:number(u.completion_tokens_details?.reasoning_tokens??u.output_tokens_details?.reasoning_tokens)};}catch{}};
      const measuredBody=response.body?.pipeThrough(new TransformStream<Uint8Array,Uint8Array>({transform(chunk,output){firstByteAt??=Date.now();pending+=decoder.decode(chunk,{stream:true});if(streaming){let end;while((end=pending.indexOf('\n'))>=0){inspect(pending.slice(0,end));pending=pending.slice(end+1);}}if(pending.length>1048576)pending='';output.enqueue(chunk);},flush(){pending+=decoder.decode();inspect(pending);pending='';}}));
      const measuredResponse=new Response(measuredBody??null,{status:response.status,statusText:response.statusText,headers:response.headers});
      let released=false;return {response:measuredResponse,release:()=>{if(!released){released=true;this.active--;
        void this.recordUsage?.({id:randomUUID(),at:startedAt,completedAt:this.clock(),traceKind:'upstream-attempt',lifecycleState:'completed',route:'team-machine/'+transport,application:'team-machine/'+transport,model:body.model,provider:'openai-compatible',executionLocation:'local',status:signal.aborted?499:response.status,isError:signal.aborted||!response.ok,stream:body.stream===true,latencyMs:this.clock()-startedAt,usageStatus:usage?'measured':'missing',costStatus:'unpriced',...(firstByteAt?{ttftMs:Math.max(0,firstByteAt-startedAt)}:{}),...(usage?{tokensInput:usage.input,tokensOutput:usage.output,tokensInputCached:usage.cached,tokensReasoning:usage.reasoning}:{})},member).catch(()=>undefined);
      }}};
