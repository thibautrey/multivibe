...
function upstream(calls: string[]): ModelBenchmarkClient {
  return {
    sources: () => ({ sources: [] }),
    model: async (modelId: string) => { calls.push(`model:${modelId}`); return { modelId, observations: [], source: "hugging-face", fetchedAt: "upstream" }; },
    leaderboard: async (datasetId: string, limit?: number) => { calls.push(`leaderboard:${datasetId}:${limit}`); return { datasetId, entries: [], source: "hugging-face", fetchedAt: "upstream" }; },
    artificialAnalysisModels: async (page?: number, access?: "free" | "full") => { calls.push(`aa-page:${access}:${page}`); return { models: [], page }; },
    artificialAnalysisModel: async (slug: string) => { calls.push(`aa-model:${slug}`); return { model: { slug } }; },
  } as unknown as ModelBenchmarkClient;
}
...
test("durable cache survives restart and avoids repeated upstream requests", async () => {
  ...
  } finally { await rm(directory, { recursive: true, force: true, maxRetries: 5, retryDelay: 20 }); }
});
