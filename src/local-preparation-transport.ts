...
export type HostPreparationOperation = {
  operation: 'install'|'start'|'download'|'import'|'test';
...
export async function readHostPreparationOperation(response:Response,input:HostPreparationOperation,signal:AbortSignal,progress:(completed:number,total:number)=>Promise<void>):Promise<{runtimeModel?:string;output?:string}> {
  ...
  let buffer='';let previous=0;let done=false;let result:{runtimeModel?:string;output?:string}={};
  const line=async(raw:string)=>{
    signal.throwIfAborted();
    if(done||!raw||raw.length>32768)throw Error('invalid_preparation_stream');
    ...
    if(event.type==='progress'){
      ...
      if(input.operation==='test'){
        if(typeof event.output!=='string'||!event.output.trim()||new TextEncoder().encode(event.output).length>4096||event.runtime_model)throw Error('invalid_preparation_result');
      ...
      if(input.operation!=='test'&&event.output!==undefined)throw Error('invalid_preparation_result');
  ...
  try{
    while(true){
      ...
      if(buffer.length>32768)throw Error('invalid_preparation_stream');
    }
