...
func run(root string, write bool) error {
	...
	dep, err := os.ReadFile(filepath.Join(root, "packaging/provider-host-dependencies.json"))
	if err != nil {
		return err
	}
	if err := json.Unmarshal(dep, &dependencies); err != nil {
		return err
	}
	for _, profile := range catalog.Profiles {
		if profile.Runtime.BackendID != "ollama-managed" {
			continue
		}
	...
	finalized, err := runtimeprofile.Finalize(catalog)
	if err != nil {
		return err
	}
	if write {
		...
		for _, name := range []string{catalogPath, fixturePath} {
	...
	if err := runtimeprofile.Validate(catalog); err != nil {
		return fmt.Errorf("catalog digests are stale: run npm run catalog:refresh: %w", err)
	}
	...
	if !bytes.Equal(raw, fixture) {
		return fmt.Errorf("catalog golden fixture differs: run npm run catalog:refresh")
	}
	return nil
}
func main() {
