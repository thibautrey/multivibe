...
func (b *cloudIntegrationBackend) Execute(_ context.Context, r runtimeExecuteRequest) (runtimeExecuteResult, error) {
	...
	if b.fail {
		return runtimeExecuteResult{}, errors.New("private runtime diagnostic")
	}
...
func (b *cloudIntegrationBackend) ExecuteStream(_ context.Context, r runtimeExecuteRequest, emit func(runtimeExecuteChunk) error) (runtimeExecutionSummary, error) {
	...
	if b.fail {
		return runtimeExecutionSummary{}, errors.New("private runtime diagnostic")
	}
	...
	for _, scenario := range []struct {
		...
		t.Run(scenario.name, func(t *testing.T) {
			...
			cloud := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				if r.Method != http.MethodPost || r.Header.Get("authorization") != "Bearer "+session.Token {
					t.Error("invalid Cloud request authentication or method")
					w.WriteHeader(401)
					return
				}
				if r.URL.Path == "/provider/v1/inference-jobs/claim" {
				...
				if r.URL.Path != "/provider/v1/inference-jobs/"+claim.JobID+suffix && r.URL.Path != "/provider/v1/inference-jobs/"+claim.JobID+"/complete" {
				...
				if json.NewDecoder(r.Body).Decode(&body) != nil {
					t.Error("invalid result JSON")
				}
				if body["leaseId"] != claim.LeaseID {
					t.Error("result lost lease binding")
				}
				receipts <- body
				w.WriteHeader(201)
				_, _ = w.Write([]byte(`{}`))
			...
			if err := sessions.replace(session, now); err != nil {
				t.Fatal(err)
			}
			...
			replay, err := openCommunityOutboundReplayStore(path)
			if err != nil {
				t.Fatal(err)
			}
			backend := &cloudIntegrationBackend{fail: scenario.fail}
			base, err := url.Parse(cloud.URL)
			if err != nil {
				t.Fatal(err)
			}
			...
			defer cancel()
			received, err := worker.claim(ctx, session)
			if err != nil || received == nil {
				t.Fatalf("claim: %v", err)
			}
			if !reflect.DeepEqual(*received, claim) {
				t.Fatal("HTTP claim changed the signed wire request")
			}
			worker.execute(ctx, *received)
			if len(backend.requests) != 1 {
				t.Fatalf("backend executions: %d", len(backend.requests))
			}
			input, _ := base64.RawURLEncoding.DecodeString(claim.Wire.Body)
			request := backend.requests[0]
			if request.ModelID != backend.CommunityCatalog()[0].CanonicalModelID || request.ExecutionID != claim.JobID || string(request.Input) != string(input) {
				t.Fatalf("runtime request lost model/job/body binding: %#v", request)
			}
			count := 1
			if scenario.stream {
				count = 2
			...
			for i := 0; i < count; i++ {
				...
				default:
					t.Fatal("missing Cloud result")
				}
				raw, err := base64.RawURLEncoding.DecodeString(receipt["body"].(string))
				if err != nil {
					t.Fatal(err)
				}
				if strings.Contains(string(raw), "private runtime diagnostic") {
					t.Fatal("runtime diagnostic leaked")
				}
				if scenario.stream {
					if receipt["sequence"] != float64(i) || receipt["final"] != (i == 1) {
						t.Errorf("stream ordering/finality: %#v", receipt)
					}
					...
					if scenario.fail {
						status, disposition = 502, "uncertain"
					}
					if receipt["status"] != status || receipt["disposition"] != disposition {
						t.Errorf("completion: %#v", receipt)
					}
					expected := `{"choices":[{"message":{"content":"hello"}}]}`
					if scenario.fail {
						expected = `{"error":{"code":"community_inference_backend_failed"}}`
					}
					if string(raw) != expected {
						t.Errorf("completion body: %q", raw)
					}
				}
			}
			...
			worker.replay, err = openCommunityOutboundReplayStore(path)
			if err != nil {
				t.Fatal(err)
			}
			worker.execute(ctx, *received)
			if len(backend.requests) != 1 {
				t.Fatal("redelivered job executed twice")
			}
			select {
			case receipt := <-receipts:
				if receipt["disposition"] != "uncertain" || receipt["status"] != float64(502) {
					t.Errorf("replay completion: %#v", receipt)
				}
			default:
				t.Fatal("missing replay completion")
			}
			if len(receipts) != 0 {
