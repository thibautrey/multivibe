...
export function OpenModelDiscovery({ compact, need: selectedNeed, expert = false, connected = [], onUse }: { compact: boolean; need?: CatalogNeed; expert?: boolean; connected?: GuidanceEntry[]; onUse?: (id:string)=>void }) {
  ...
  useEffect(()=>{
    ...
    const load=()=>void api(`/admin/model-recommendations?need=${effectiveNeed}&sort=${sort}&host=local${benchmark ? `&benchmark=${encodeURIComponent(benchmark)}` : ''}${Number(memoryBudget)>0 ? `&memory_gib=${encodeURIComponent(memoryBudget)}` : ''}`,{signal:controller.signal}).then((value:Result)=>{if(!controller.signal.aborted){setResult(value);setError(false);}}).catch(()=>{if(!controller.signal.aborted)setError(true);});
  ...
  return <section className={`models-open-discovery${expert ? '' : ' models-picker'}${compact ? ' is-beginner' : ''}`} aria-label="Open model discovery">
    ...
    {result?.benchmarks && <p className="models-benchmark-coverage" role="status">Benchmark cache: {result.benchmarks.coverage.cachedModels} / {result.benchmarks.coverage.totalModels} task models checked{result.benchmarks.coverage.warming ? ' · Collecting more in the background' : ''}. {result.memory?.freeHostMiB != null ? `${(result.memory.freeHostMiB/1024).toFixed(1)} GiB RAM available` : 'Current free memory unavailable'}{result.memory?.freeDeviceMiB != null ? ` · ${(result.memory.freeDeviceMiB/1024).toFixed(1)} GiB accelerator memory available` : ''}.</p>}
