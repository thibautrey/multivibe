      ...
      {activeView === "overview" && (
        <div id="trace-view-overview" role="tabpanel" aria-labelledby="trace-tab-overview" className="trace-view-content">
          <WidgetGrid storageKey="activity" label="Activity metrics">
            <Metric widgetId="client-requests" title="Client requests" value={`${requestCount}`} detail={`${providerAttemptCount} provider attempts · ${rangeLabel}`} />
            <Metric widgetId="client-error-rate" title="Client error rate" value={pct(traceStats.totals.errorRate)} detail={`${errorCount} final failures`} tone={traceStats.totals.errorRate > 0.05 ? "warning" : "default"} />
            <Metric widgetId="avg-latency" title="Avg latency" value={`${Math.round(traceStats.totals.latencyAvgMs)}ms`} detail="End-to-end response time" />
            <Metric widgetId="total-cost" title="Total cost" value={usd(traceStats.totals.costUsd)} detail={`No-cache estimate: ${usd(traceStats.totals.costUsdWithoutCache)} · ${traceStats.totals.requestsWithCost}/${providerAttemptCount} attempts priced`} tone={traceStats.totals.unpricedRequests > 0 ? "warning" : "default"} />
          </WidgetGrid>
      ...
      {activeView === "usage" && (
        <div id="trace-view-usage" role="tabpanel" aria-labelledby="trace-tab-usage" className="trace-view-content">
          <WidgetGrid storageKey="activity-usage" label="Usage metrics">
            <Metric widgetId="input-tokens" title="Input tokens" value={formatTokenCount(traceStats.totals.tokensInput)} detail={`${traceStats.totals.requestsWithUsage}/${providerAttemptCount} attempts measured`} tone={usageCoverage < 1 ? "warning" : "default"} />
            <Metric widgetId="output-tokens" title="Output tokens" value={formatTokenCount(traceStats.totals.tokensOutput)} detail="Generated by providers" />
            <Metric widgetId="total-cost" title="Total cost" value={usd(traceStats.totals.costUsd)} detail={`No-cache estimate: ${usd(traceStats.totals.costUsdWithoutCache)} · ${traceStats.totals.requestsWithCost}/${providerAttemptCount} attempts priced`} tone={pricingCoverage < 1 ? "warning" : "default"} />
            <Metric widgetId="inference-speed" title="Inference speed" value={formatTokenRate(traceStats.totals.inferenceTokensPerSecond)} detail={`${traceStats.totals.inferenceRequests} measurable requests`} />
          </WidgetGrid>
