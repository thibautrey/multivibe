...
export function ModelVariantsTable({row, memory, supported, onPrepare}: {row:Row;memory?:MemoryAvailability;supported:boolean;onPrepare:(id:string)=>void}) {
  ...
  return <details className="models-variants" open={open} onToggle={event=>setOpen(event.currentTarget.open)}><summary>Quantizations and variants ({models.length} repositories)</summary>
    {open && <><p>Original: <strong>{row.model.id}</strong>. Conversions follow declared Hugging Face ancestry; fine-tunes remain separate models.</p>
    ...
    <div className="models-variants-scroll"><table><thead><tr><th>Variant / publisher</th><th>Quantization</th><th>Weight size</th><th>Memory fit</th><th>Downloads</th><th>Action</th></tr></thead><tbody>{models.flatMap(model=>{
