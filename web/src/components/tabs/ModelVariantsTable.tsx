import {memoryStatusLabels} from '../../../../src/model-memory-status';
import type {MemoryEstimateReport} from '../../../../src/model-discovery-memory';
import {exceedsWeightBudget,discoveryMemoryFit} from '../../../../src/model-memory-budget';
...
export function ModelVariantsTable({row, memory, supported, onPrepare}: {row:Row;memory?:MemoryAvailability;supported:boolean;onPrepare:(id:string)=>void}) {
  const [memoryReports,setMemoryReports]=useState<Record<string,MemoryEstimateReport>>({});
  ...
  useEffect(()=>{
    if(!choice)return;const controller=new AbortController();let timer:ReturnType<typeof setTimeout>|undefined;
    const load=()=>void api(`/admin/model-memory?model=${encodeURIComponent(row.model.id)}&variant=${encodeURIComponent(choice.model.id)}`,{signal:controller.signal}).then((value:{report?:MemoryEstimateReport;pending:boolean})=>{
      if(controller.signal.aborted)return;
      if(value.report)setMemoryReports(previous=>({...previous,[choice.model.id]:value.report!}));
      if(value.pending)timer=setTimeout(load,value.report?.reason==='temporary_failure'?30000:2000);
    }).catch(()=>{if(!controller.signal.aborted)setMemoryReports(previous=>({...previous,[choice.model.id]:{reason:'temporary_failure',estimates:[],checkedAt:new Date().toISOString()}}));});
    load();return()=>{controller.abort();if(timer)clearTimeout(timer);};
  },[choice?.model.id,row.model.id]);
  const artifactPrediction=choice && memoryReports[choice.model.id]?.estimates.find(e=>e.artifact===choice.artifact.name);
  const initialPrediction=choice && row.variants.find(v=>v.model.id===choice.model.id && v.memory.artifact===choice.artifact.name)?.memory;
  const prediction=initialPrediction?.source==='runtime'?initialPrediction:artifactPrediction?{...artifactPrediction,state:discoveryMemoryFit(artifactPrediction.requiredMiB,memory)}:initialPrediction;
  const estimationReason=choice && (memoryReports[choice.model.id]?.artifactReasons?.[choice.artifact.name] ?? memoryReports[choice.model.id]?.reason ?? row.variants.find(v=>v.model.id===choice.model.id)?.estimateStatus ?? 'queued');
  return <section className="model-download-options"><h4>Download options</h4>
    {choice && <div className="model-variant-choice"><label><span className="sr-only">Model variant</span><select aria-label="Model variant" value={`${choice.model.id}:${choice.artifact.name}`} onChange={event=>setSelection(event.target.value)}>{selectorOptions.map(({model,artifact})=><option key={`${model.id}:${artifact.name}`} value={`${model.id}:${artifact.name}`}>{artifact.format} · {artifact.quantization} · {artifact.bytes===null?'Size unknown':`${(artifact.bytes/1073741824).toFixed(2)} GiB`} · {model.id.split('/')[0]}</option>)}</select></label>
      ...
      <div className="model-variant-footer"><span className={prediction?.state==='compatible'?'model-fit-positive':'muted'}>{prediction?.state==='compatible'?'Estimated to fit':prediction?.state==='insufficient'||exceedsWeightBudget(choice.artifact.bytes,memory)?'Exceeds memory limit':prediction?'Memory fit unknown':memoryStatusLabels[estimationReason || 'queued']}</span><a className="btn ghost" href={choice.model.url} target="_blank" rel="noreferrer">View files ↗</a>{supported && !row.model.gated && <button className="btn primary" onClick={()=>onPrepare(row.model.id)}>Check Host setup</button>}</div>
