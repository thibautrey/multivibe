    ...
    <div className="models-layout">
      ...
      <div className="models-results" aria-busy={loading}>
        ...
        <ul className="models-list">{filtered.slice(currentPage * PAGE_SIZE, (currentPage + 1) * PAGE_SIZE).map(model => {
          ...
          return <li key={model.id}><div className="models-row"><div className="models-identity"><span className="models-monogram" aria-hidden="true">{model.name.replace(/^(hf|openrouter):/, '').slice(0, 2).toUpperCase()}</span><div className="models-copy"><strong>{model.name}</strong><code>{model.id}</code><span className="muted">{[...new Set(model.routes.map(route => route.label))].join(' · ')}</span></div></div><span className={`models-status${ready ? ' is-ready' : ''}`}><i />{ready ? 'Ready to use' : 'Setup needed'}</span><button className={`btn ${ready ? '' : 'ghost'}`} disabled={connecting} aria-label={`${actionLabel(preferred)}: ${model.name}`} onClick={() => useRoute(preferred)}>{actionLabel(preferred)}<span aria-hidden="true"> →</span></button></div>
