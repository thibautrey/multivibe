...
import {createMemoryEstimationQueue} from '../../src/model-memory-queue';
import {modelMemoryRequest} from '../../src/model-memory-request';
...
const memoryQueue=createMemoryEstimationQueue({path:path.join(os.tmpdir(),'multivibe-demo-memory-queue-v2.json')});
const evidenceFor = createRecommendationEvidence(benchmarkClient, Date.now, undefined,memoryQueue);
...
export function demoApiPlugin(): Plugin {
  return {
    ...
    configureServer(server) {
      ...
      server.httpServer?.once("close", ()=>{stopCatalog();void memoryQueue.close();});
      ...
      server.middlewares.use((req, res, next) => {
        ...
        if(path==='/admin/model-memory' && req.method==='GET'){
          const params=new URL(req.url!,'http://demo.invalid').searchParams;
          void modelMemoryRequest(memoryQueue,loadOpenModelCatalog,params.get('model')??'',params.get('variant')??'').then(result=>{res.setHeader('content-type','application/json');res.end(JSON.stringify(result));}).catch(()=>{res.statusCode=400;res.end('{}');});return;
        }
