...
For zero-touch company deployment, use the platform installer's
`--managed-profile` option (`-ManagedProfilePath` on Windows) with a private,
single-target profile generated by the MultiVibe Team dashboard. See
`docs/managed-team-deployment.md` in the source repository. The profile is a
short-lived bootstrap only; never place an employee API key or an instance
private key in MDM configuration.
